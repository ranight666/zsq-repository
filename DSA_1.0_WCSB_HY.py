...
Modification notes:
    This version was adapted for focal-depth determination of earthquakes
    in the Western Canadian Sedimentary Basin (WCSB). The main modifications
    include adjustments to the direct-arrival phase extraction strategy and
    the preliminary depth-range selection strategy, aiming to improve the
    robustness of the Depth-Scanning Algorithm (DSA) for real-data applications
    in the WCSB.
...
def load_settings():
    ...
    try:
        #SETTINGS = pd.read_csv('./DSA_v1/DSA_SETTINGS_WCSB_general.txt', delim_whitespace=True, index_col='PARAMETER')
        ...
        SETTINGS = pd.read_csv('./DSA_v1/DSA_SETTINGS_Montney_Bei.txt', delim_whitespace=True, index_col='PARAMETER')
...
def GetCrustInterfaceDepths( velModel ):
    ...
    velMod = pd.read_csv('./vel_model/'+str(velModel)+'.nd',
                         delim_whitespace=True, header=None,
                         names=['TopDepth', 'Vp', 'Vs', 'Rho', 'Qp', 'Qs'])
...
taup_create.build_taup_model( './vel_model/'+''+str(velModel)+'.nd' )
...
# ax11.scatter(prelimSolution_0, sumAvgArrTimeDiffResGlobal[np.int(prelimSolution)-1],
#              s=30, marker='s', facecolors='gray', edgecolor='gray', zorder=100)
ax11.scatter(prelimSolution_0, sumAvgArrTimeDiffResGlobal[np.int(prelimSolution)-np.int(scanDepthFrom)],
             s=30, marker='s', facecolors='gray', edgecolor='gray', zorder=100)
...
ax11.scatter(prelimSolution, sumAvgArrTimeDiffResGlobal_2[np.int(prelimSolution)-np.int(scanDepthFrom)],
...
if len( wellBehavedStationId ) < 1:
    ...
    if verboseFlag == 1:
        ...
        ax00.scatter(prelimSolution_0, sumAvgArrTimeDiffResGlobal[np.int(prelimSolution)-np.int(scanDepthFrom)],
        ...
        ax00.scatter(prelimSolution, sumAvgArrTimeDiffResGlobal_2[np.int(prelimSolution)-np.int(scanDepthFrom)],
