...
def load_settings():
    ...
    try:
        SETTINGS = pd.read_csv(r'c:\DSA0\DSA_SETTINGS_WCSB_general.right.txt', delim_whitespace=True, index_col='PARAMETER')
...
def GetCrustInterfaceDepths( velModel ):
    ...
    velMod = pd.read_csv('C:\\DSA0\\vel_model\\'+str(velModel)+'.nd',
                         delim_whitespace=True, header=None,
                         names=['TopDepth', 'Vp', 'Vs', 'Rho', 'Qp', 'Qs'])
...
taup_create.build_taup_model( "C:\\DSA0\\vel_model\\"+''+str(velModel)+'.nd' )
...
vel_file = os.path.join('C:\\DSA0\\vel_model', str(velModel) + '.nd')
print("[DEBUG] velModel MD5 =", file_md5(vel_file))
#%%-- Step 1 to 4 of DSA
for ist in range( numSt ):     #单核运行
    ...
    #%%-- Read waveform data                  
    infileE = open(r'{0}\\{1}'.format( dataPath,wfFiles[ist*3+0] ) )
    infileN = open(r'{0}\\{1}'.format( dataPath, wfFiles[ist*3+1] ) )
    infileZ = open(r'{0}\\{1}'.format( dataPath, wfFiles[ist*3+2] ) )
    print( infileE.name )
    #%%--
...
#        print(stT0)    
    ...
    print( "calOnsetS=", calOnsetS )
        ...
        # === 调试：初至拾取结果 ===
    print(f"\n[DEBUG_STATION] Station {ist} : {stRawZ[0].stats.sac.kstnm}")
    print(f"  onsetZNor={onsetZNor:.3f} s, onsetRNor={onsetRNor:.3f} s, onsetTNor={onsetTNor:.3f} s")
    print(f"  findOnsetIdxFlagZ={findOnsetIdxFlagZ}, findOnsetIdxFlagR={findOnsetIdxFlagR}, findOnsetIdxFlagT={findOnsetIdxFlagT}")
    ...
    for tmp_depth in np.arange( srcDepthScanBeg, srcDepthScanEnd, srcDepthScanInc):
        ...
        arrivals, rays = subDeleteRefractedWave( crustInterfaceDepths, srcDepth, arrivals, rays )
...
#                            fig=plt.figure(figsize=(1,1), linewidth=0.01, dpi=200 ))
        ...
        # synthetic data calculated by using TauP package.
        ...
        depthCandidateArrTaupDiffGlobalR[ist][indexCounter].append(matchedTaupTimeDiffR)
        depthCandidateArrTaupDiffGlobalT[ist][indexCounter].append(matchedTaupTimeDiffT)
        depthCandidateArrTaupDiffGlobalZ[ist][indexCounter].append(matchedTaupTimeDiffZ)

        depthCandidateArrdtGlobalR[ist][indexCounter].append(np.array(matchedPickedTimeDiffR) - np.array(matchedTaupTimeDiffR))
        depthCandidateArrdtGlobalT[ist][indexCounter].append(np.array(matchedPickedTimeDiffT) - np.array(matchedTaupTimeDiffT))
        depthCandidateArrdtGlobalZ[ist][indexCounter].append(np.array(matchedPickedTimeDiffZ) - np.array(matchedTaupTimeDiffZ))
...
for i in range( len(scanDepthMembers) ):
    if sumGlobal[i] >= thresholdMaxNumb_2:
        ...
        tmp_matched_dt = []
        for j in range(numSt):
            tmp_matched_dt.extend(list(chain.from_iterable(depthCandidateArrdtGlobalR[j][i])))
            tmp_matched_dt.extend(list(chain.from_iterable(depthCandidateArrdtGlobalT[j][i])))
            tmp_matched_dt.extend(list(chain.from_iterable(depthCandidateArrdtGlobalZ[j][i])))
...
debug_out = outfilePath + 'step3_debug_data.npz'

# 将需要对比的变量转为 numpy 数组（若尚未是）
scanDepthMembers_arr = np.array(scanDepthMembers)
sumGlobal_arr = np.array(sumGlobal)
sumAvgArrTimeDiffResGlobal_arr = np.array(sumAvgArrTimeDiffResGlobal)
sumAvgArrTimeDiffResGlobal_2_arr = np.array(sumAvgArrTimeDiffResGlobal_2)
totNumMatPhaGlobal_arr = np.array(totNumMatPhaGlobal)   # shape (numSt, numScanDepth)
# 可选：保存每个站点的平均残差
avgArrTimeDiffResEachStationSum_arr = np.array(avgArrTimeDiffResEachStationSum)  # shape (numSt, numScanDepth)

np.savez(debug_out,
         scanDepthMembers=scanDepthMembers_arr,
         sumGlobal=sumGlobal_arr,
         sumAvgArrTimeDiffResGlobal=sumAvgArrTimeDiffResGlobal_arr,
         sumAvgArrTimeDiffResGlobal_2=sumAvgArrTimeDiffResGlobal_2_arr,
         prelimSolution=prelimSolution,
         prelimSolution_0=prelimSolution_0,
         thresholdMaxNumb=thresholdMaxNumb,
         totNumMatPhaGlobal=totNumMatPhaGlobal_arr,
         avgArrTimeDiffResEachStationSum=avgArrTimeDiffResEachStationSum_arr)

print(f"Step 3 debug data saved to {debug_out}")
...
if len( wellBehavedStationId ) < 1:
    ...
    for isw in range( numWellBehavedStation ):
        ...
        if depthCandidate - 1 < scanDepthFrom:
            srcDepth_b = scanDepthFrom
        else:
            srcDepth_b = depthCandidate - 1

        if depthCandidate + 1 > scanDepthTo:
            srcDepth_e = scanDepthTo
        else:
            srcDepth_e = depthCandidate + 1

        for srcDepth in np.arange( srcDepth_b, srcDepth_e, 0.1):
            taupOriginalName = []
            arrivalsTimeDiffP = []
            arrivalsTimeDiffS = []
            ...
            for i in range( len( depthCandidatePhaDigNameGlobalZ[ist][idx][0] ) ):
                for j in range(len(arrivals)):
                    # if ( depthCandidatePhaDigNameGlobalZ[ist][idx][0][i] == arrivals[j].name ) and \
                    if ( depthCandidatePhaOrgNameGlobalZ[ist][idx][0][i] == taupOriginalName[j] ) and \
                        math.fabs( depthCandidateArrDiffGlobalZ[ist][idx][0][i] - arrivalsTimeDiffP[j])<= arrTimeDiffTole:
                        tmp += ( depthCandidateArrDiffGlobalZ[ist][idx][0][i] - arrivalsTimeDiffP[j])**2
                        count += 1 # the number of matched phases for station ist at depth idx (obtained in step 3)
            if count > 0:
                tmp3 += tmp
            ...
            for i in range( len( depthCandidatePhaDigNameGlobalR[ist][idx][0] ) ):
                for j in range(len(arrivals)):
                    # if (depthCandidatePhaDigNameGlobalR[ist][idx][0][i] == arrivals[j].name) and \
                    if (depthCandidatePhaOrgNameGlobalR[ist][idx][0][i] == taupOriginalName[j] ) and\
                        math.fabs( depthCandidateArrDiffGlobalR[ist][idx][0][i] - arrivalsTimeDiffP[j])<= arrTimeDiffTole:
            ...
            for i in range( len( depthCandidatePhaDigNameGlobalT[ist][idx][0] ) ):
                for j in range(len(arrivals)):
                    # if (depthCandidatePhaDigNameGlobalT[ist][idx][0][i] == arrivals[j].name) and \
                    if ( depthCandidatePhaOrgNameGlobalT[ist][idx][0][i] == taupOriginalName[j] ) and \
                        math.fabs( depthCandidateArrDiffGlobalT[ist][idx][0][i] - arrivalsTimeDiffS[j])<= arrTimeDiffTole:
            ...
            # if count3 > 0:
            #     sumRmsRTZ = math.sqrt( tmp3/count3 )

            #sumRmsRTZ = ( rmsZ[-1] )
            rmsRTZ.append(sumRmsRTZ)
            exactDepthRTZ.append( srcDepth )
            tmp_match_count.append(count3)
            
        
        #-- find out the best depth solution
        idx_min = np.argmin(rmsRTZ)
        depthSolutionRTZ = exactDepthRTZ[ idx_min ]
        Sta_match[ist] = tmp_match_count[idx_min]
        ...
        with open( '{0}'.format( outPath ),  mode='a', newline='' ) as resultsFile:
            ...
            writer.writerow(['{0}'.format( nameEachStation[ist][0] ),
                             ...
                             '{0}'.format(tmp_match_count),
                             '{0}'.format( format( depthSolutionRTZ, ".2f" ) ),
                             '{0}'.format( exactDepthRange ),
                             '{0}'.format( rmsRTZ ),
                             '{0}'.format( format( rmsRTZ[idx_min], ".2f")) ])

        Sta_dt_final[ist] = rmsRTZ[idx_min]

        Sta_dep_final[ist] = depthSolutionRTZ
    ...
    if verboseFlag == 1:
        ...
        ax00.scatter(prelimSolution_0, sumAvgArrTimeDiffResGlobal[int(prelimSolution)-int(scanDepthFrom)],
        ...
        ax00.scatter(prelimSolution, sumAvgArrTimeDiffResGlobal_2[int(prelimSolution)-int(scanDepthFrom)],
...
df = pd.DataFrame()
df['station'] = Sta_stanm
df['lon'] = Sta_stalon
df['lat'] = Sta_stalat
df['dist'] = Sta_distkm
df['dt_R'] = Sta_timediffR
df['dt_T'] = Sta_timediffT
df['dt_Z'] = Sta_timediffZ
df['dt_final'] = Sta_dt_final
df['n_match'] = Sta_match
df['dep'] = Sta_dep_final
df['best'] = Sta_best
outPath2 = str(outfilePath)+'/dt_cmp'
df.to_csv('{0}.csv'.format(outPath2),index=False)
#%%       
resultsFile.close()
...
np.save('C:/DSA0/test/sumGlobal_parallel.npy', sumGlobal)
np.save('C:/DSA0/test/sumAvgArrTimeDiffResGlobal_parallel.npy', sumAvgArrTimeDiffResGlobal)
